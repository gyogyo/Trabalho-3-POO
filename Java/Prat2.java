...
class Prat2{
	...
	ArrayList<Integer> rnglist = new ArrayList<Integer>(8); //Lista para uso na ordenaçao aleatoria de times.

	//Alguns personagens de RPG.
	Characters.Character[] Players = new Characters.Character[8];
	...
	Teams[0] = new Team("Red",Color.red);
	Teams[1] = new Team("Blue",Color.blue);

	Item[] Items = new Item[24];

	Items[0] = new Armor("Escudo de Ferro", 100, 3, 30.0);
	...
	Items[6] = new Weapon("Espada Lendaria", 600, 7, 7.0);
	Items[7] = new Weapon("Osafune", 300, 3, 5.0);
	Items[8] = new Weapon("Ame No Murakamo", 500, 5, 6.0);
	...
	for(int i=0; i<8; i++) rnglist.add(i); //Colocar ints em uma deque para evitar repeticao.
	while(rnglist.size()>1){
	int i = (int) (Math.random()*rnglist.size()); //Puxar ints aleatoriamente para sortear os personagens.
	int j = rnglist.get(i);
	rnglist.remove(i); //Apagar int ja selecionado.
	i = (int) (Math.random()*rnglist.size()); //Puxar personagem oponente.
	int k = rnglist.get(i);
	rnglist.remove(i); //Apagar selecionado.
